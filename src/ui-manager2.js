...
class UIManager {
    constructor(config) {
        ...
        this.onQueueDrained = null; // Callback when typewriter finishes rendering
    ...
    init() {
        ...
        // TAB SWITCH FIX: When user returns to tab, handle queued tokens
        document.addEventListener('visibilitychange', () => {
            if (document.visibilityState === 'visible') {
                // Flush any accumulated tokens
                if (this.typewriterQueue.length > 0) {
                    this._flushQueue();
                }

                // CRITICAL FIX: If network stream is still active but loop was killed (by flush), restart it.
                if (!this.isNetworkStreamDone && !this.typewriterTimer) {
        ...
        const tick = (timestamp) => {
            ...
            if (timestamp - this._lastFrameTime >= 8) {
                ...
                if (this.typewriterQueue.length > 0) {
                    ...
                    this.typewriterTimer = null;
                    this.currentAIMessageElement = null;
                    // Fire callback to re-enable input
    ...
    _flushQueue() {
        ...
        // If network done and we just flushed everything, fire the callback
        if (this.isNetworkStreamDone && this.onQueueDrained) {
            this.onQueueDrained();
            this.onQueueDrained = null;
        }
    }
    ...
    finishAIMessage() {
        ...
        // FIX: If tab is hidden OR the typewriter loop is dead (e.g. killed by flush),
        // we must force flush any remaining tokens immediately to complete the message.
        if (document.visibilityState === 'hidden' || !this.typewriterTimer) {
            this._flushQueue();
            this.currentAIMessageElement = null;
        }

        // If visible and loop is running, do NOTHING else. The _startTypewriterLoop will handle the rest:
        // 1. Drain queue
